...
**2026-09-14 · The bundle probe broke on a tmpdir with a space in it, and read that as a FAIL of the bundle.** It spawned the child with `shell: true` and unquoted args, so `C:\Users\John Smith\AppData\Local\Temp\mcpbprobe-xxxx\server\index.js` was re-split by the shell. Reproduced by pointing `TEMP` at a spaced directory: `Cannot find module 'C:\Users\abish\AppData\Local\Temp\probe'`, exit 1, printed as a probe failure. Nothing was wrong with the artifact — the instrument fails on the majority of Windows user profiles, which is the population it exists to serve. `pack-probe.mjs` already quoted for exactly this reason. **Fixed**: every arg is quoted; re-run against the spaced tmpdir, PASS.
